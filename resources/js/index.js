	...
	try {
		...
		for (let i = 0; i < 8; i++) {
			...
			if (description == 'Sunny') {
				...
				predictImg.src = 'resources/svg/weather-Clear.png';
			}
...
// mobile sizing
const removeClass = () => {
	const wholeBox = document.getElementById('weather-box');
	const hourlyPre = document.getElementsByClassName('hourlyPre');
	let screenWidth = window.innerWidth;
	if (screenWidth < 400) {
		wholeBox.classList.remove('inline-flex');
		for (const hourlyValue of hourlyPre) {
			hourlyValue.style.width = document.getElementById(
				'hourlyPredictionChart'
			).style.width;
		}
	} else {
		wholeBox.classList.add('inline-flex');
		for (const hourlyValue of hourlyPre) {
			hourlyValue.style.width = '504px';
		}
	}
};
